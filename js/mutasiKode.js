...
function updateDataSourceIndicator() {
  ...
  const sourceText =
    currentDataSource === 'penjualanAksesoris' ? 'Transaksi Penjualan' : 'Mutasi Kode'
...
async function mutateSelectedKodes() {
  try {
    ...
    $('#btnMutasiSelected')
      .prop('disabled', true)
      .html(`<i class="fas fa-exchange-alt me-2"></i>Mutasi Terpilih`)
...
function renderKodeTable(data, type) {
  ...
  if (data.length === 0) {
    ...
    tableBody.html(
      `<tr><td colspan="${colCount}" class="text-center">Tidak ada data kode</td></tr>`
    )
    return
  }
  ...
  data.forEach((item) => {
    ...
    const row = `
      <tr data-id="${item.id}">
        <td>
          <input type="checkbox" class="form-check-input kode-checkbox" data-id="${
            item.id
          }" data-type="${type}">
        </td>
...
function attachTableEventHandlers(type) {
  ...
  $(document).off(
    'change',
    `#table${type.charAt(0).toUpperCase() + type.slice(1)}Kode .kode-checkbox`
  )
  $(document).on(
    'change',
    `#table${type.charAt(0).toUpperCase() + type.slice(1)}Kode .kode-checkbox`,
    function () {
      ...
      console.log(
        `Checkbox changed: ${id}, type: ${checkboxType}, checked: ${$(this).is(':checked')}`
      )
  ...
  $(document).on(
    'click',
    `#table${type.charAt(0).toUpperCase() + type.slice(1)}Kode .btn-detail`,
    function () {
      const id = $(this).data('id')
...
async function confirmRestoreKode() {
  const confirmed = await showConfirm(
    'Apakah Anda yakin ingin mengembalikan kode yang dipilih ke status aktif?'
  )
  if (confirmed) {
...
function exportToExcel(data, filename, sheetName = 'Data') {
  try {
    if (typeof XLSX === 'undefined') {
      showAlert(
        'Library Excel tidak tersedia. Pastikan XLSX library sudah dimuat.',
        'Error',
        'error'
      )
      return
...
function initializeEventHandlers() {
  ...
  $('#validasiHapusForm').on('submit', async function (e) {
    ...
    $('#btnKonfirmasiHapus')
      .prop('disabled', true)
      .html('<i class="fas fa-spinner fa-spin me-2"></i>Memvalidasi...')
    ...
    try {
      ...
      $('#btnKonfirmasiHapus')
        .prop('disabled', false)
        .html('<i class="fas fa-trash me-2"></i>Konfirmasi Hapus')
    }
  })
